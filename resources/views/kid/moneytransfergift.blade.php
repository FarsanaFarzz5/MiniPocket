...
<head>
  <meta charset="UTF-8">
  <title>Pay from Gift Savings - Mini Pocket</title>
...
<body>
  <div class="container">
    <div class="inner-container">
      ...
      <!-- Optional Reason -->
      <form method="POST" action="{{ route('kid.send.gift') }}">
        @csrf
        <input type="hidden" name="amount" id="hiddenAmount" value="0">
        <input type="text" name="description" placeholder="Gift Reason (optional)" class="reason-box">
  ...
  <script>
    ...
    const SEND_URL = "{{ route('kid.send.gift') }}"; // 👈 fixed route
    const CSRF_TOKEN = "{{ csrf_token() }}";
  </script>

  <script src="{{ asset('assets/js/moneytransfergift.js') }}"></script>
</body>
</html>
